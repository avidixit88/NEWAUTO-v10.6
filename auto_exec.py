...
def _persist_state_best_effort(state: Dict[str, Any]) -> None:
    """Best-effort local persistence for autoexec state.

    - Never raises.
    - Uses a lightweight throttle to avoid excessive disk writes.
    - Disk persistence is a convenience layer; broker truth remains authoritative.
    """
    try:
        now = _now_et()
        # throttle: at most once every 5 seconds
        last = str((state or {}).get("_disk_persist_last", "") or "")
        if last:
            try:
                ...
                pass
        ok = state_store.save_state(state)
        state["_disk_persist_last"] = now.isoformat()
        state["_disk_persist_ok"] = bool(ok)
    except Exception:
        pass
...
def _normalize_state_schemas(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return state
    # Best-effort persist on every state access (keeps disk state warm across reruns)
    _persist_state_best_effort(state)
...
def _get_state() -> Dict[str, Any]:
    ...
    # Best-effort disk restore: if session_state is empty (common after a Community Cloud sleep/restart),
    # try to reload the last persisted state from local disk. We never overwrite auth that may have been
    # created by the OAuth flow in app.py.
    try:
        if not existing or (isinstance(existing, dict) and not existing.get("lifecycles") and not existing.get("pool_reserved") and not existing.get("activity_log")):
            disk = state_store.load_state()
            if isinstance(disk, dict) and disk:
                # Merge: disk provides execution state; session/auth (if any) wins.
                merged = dict(disk)
                # Preserve any auth already present in session_state.
                if isinstance(existing_auth, dict) and existing_auth:
                    merged["auth"] = existing_auth
                existing = merged
                existing_auth = merged.get("auth", {}) if isinstance(merged.get("auth", {}), dict) else existing_auth
                # Keep a breadcrumb for debugging.
                merged["_loaded_from_disk"] = True
    except Exception:
        pass
    ...
    state: Dict[str, Any] = {
        ...
        "broker_status": existing.get("broker_status", {}) if isinstance(existing, dict) else {},
    }
...
def _broker_ready(cfg: AutoExecConfig, state: Dict[str, Any]) -> Tuple[bool, Optional[ETradeClient], str, str]:
    ...
    if client is None:
        state.setdefault('broker_status', {})
    state.setdefault('broker_status', {}).update({'ready': False, 'reason': 'missing_oauth', 'ts': _now_et().isoformat()})
    st.session_state['autoexec'] = state
    return False, None, account_id_key, "missing_oauth"
    if not account_id_key:
        state.setdefault('broker_status', {})
    state.setdefault('broker_status', {}).update({'ready': False, 'reason': 'missing_account_id', 'ts': _now_et().isoformat()})
    st.session_state['autoexec'] = state
    ...
    try:
        ...
        warn = ""
    state.setdefault('broker_status', {})
    state.setdefault('broker_status', {}).update({'ready': True, 'reason': warn or 'ok', 'ts': _now_et().isoformat()})
    st.session_state['autoexec'] = state
...
def reconcile_and_execute(
    ...
    # Persist state after reconciliation (best-effort).
    try:
        st.session_state["autoexec"] = state
        _persist_state_best_effort(state)
    except Exception:
...
def try_send_entries(cfg: AutoExecConfig, allow_opening: bool, allow_midday: bool, allow_power: bool, fetch_last_price_fn) -> None:
    ...
    # Persist state after entry evaluation/placements (best-effort).
    try:
        st.session_state["autoexec"] = state
        _persist_state_best_effort(state)
    except Exception:
